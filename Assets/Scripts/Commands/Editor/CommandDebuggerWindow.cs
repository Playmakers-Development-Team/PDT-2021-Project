...
namespace Commands.Editor
{
    public class CommandDebuggerWindow : EditorWindow
    {
        ...
        private readonly Dictionary<Assembly, List<Type>> assemblyCommandTypes =
            new Dictionary<Assembly, List<Type>>();
        private readonly Dictionary<Assembly, bool> assemblyFoldouts = new Dictionary<Assembly, bool>();
        ...
        private void OnEnable()
        {
            ...
            foreach (Assembly assembly in assemblies)
            {
                ...
                if (commandTypes.Count > 0)
                {
                    ...
                    assemblyFoldouts.Add(assembly, false);
                }
            }
        }
        ...
        private void OnGUI()
        {
            ...
            foreach (var (assembly, commandTypes) in assemblyCommandTypes)
            {
                assemblyFoldouts[assembly] = EditorGUILayout.Foldout(assemblyFoldouts[assembly], assembly.GetName().Name);

                if (!assemblyFoldouts[assembly])
                    continue;
                
                foreach (var commandType in commandTypes)
                    CommandTypeGUI(commandType);
            }
            
            GUILayout.EndScrollView();
        }

        private void CommandTypeGUI(Type commandType)
        {
            ...
            if (mainConstructor != null)
            {
                ...
                for (int i = 0; i < parameters.Length; i++)
                {
                    ...
                    if (parameterType == typeof(int))
                    {
                        parameterValues[i] ??= 0;
                        parameterValues[i] =
                            EditorGUILayout.IntField(parameterName, (int)parameterValues[i]);
                    }
                    else if (parameterType == typeof(float))
                    {
                        parameterValues[i] ??= 0;
                        parameterValues[i] =
                            EditorGUILayout.FloatField(parameterName, (float)parameterValues[i]);
                    }
                    else if (parameterType == typeof(string))
                    {
                        parameterValues[i] ??= string.Empty;
                        parameterValues[i] =
                            EditorGUILayout.TextField(parameterName, (string)parameterValues[i]);
                    }
                    else if (parameterType == typeof(Vector2))
                    {
                        parameterValues[i] ??= new Vector2();
                        parameterValues[i] =
                            EditorGUILayout.Vector2Field(parameterName, (Vector2)parameterValues[i]);
                    }
                    else if (parameterType == typeof(Vector2Int))
                    {
                        parameterValues[i] ??= new Vector2Int();
                        parameterValues[i] =
                            EditorGUILayout.Vector2IntField(parameterName, (Vector2Int)parameterValues[i]);
                    }
                    else if (parameterType == typeof(Vector3))
                    {
                        parameterValues[i] ??= new Vector3();
                        parameterValues[i] =
                            EditorGUILayout.Vector3Field(parameterName, (Vector3)parameterValues[i]);
                    }
                    else if (typeof(Ability).IsAssignableFrom(parameterType))
                    {
                        parameterValues[i] = EditorGUILayout.ObjectField(parameterName,
                            (UnityEngine.Object)parameterValues[i], typeof(Ability), false);
                    }
                    else if (typeof(UnityEngine.Object).IsAssignableFrom(parameterType))
                    {
                        parameterValues[i] = EditorGUILayout.ObjectField(parameterName,
                            (UnityEngine.Object)parameterValues[i], typeof(ScriptableObject), true);
                    }
                    else if (typeof(IUnit).IsAssignableFrom(parameterType))
                    {
                        parameterValues[i] = EditorGUILayout.ObjectField(parameterName,
                            (UnityEngine.Object)parameterValues[i], typeof(GridObject), true);
                    ...
                    {
                        parameterValues[i] = EditorGUILayout.ObjectField(parameterName,
                            (UnityEngine.Object)parameterValues[i], parameterType, true);
                    }
